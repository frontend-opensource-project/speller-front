...
const createTracker: CreateTracker = (event, action) => params => {
  // 3. gtag 함수가 로드되지 않았을 경우를 대비해 방어 코드를 추가합니다.
  if (typeof window.gtag === 'function') {
    // GA4의 표준 이벤트 전송 방식입니다.
    // 첫 번째 인자는 'event', 두 번째가 이벤트 이름(action), 세 번째가 파라미터입니다.
    window.gtag('event', action, params)
  } else {
    // gtag가 없는 경우 콘솔에 경고를 남겨 디버깅을 돕습니다.
    console.warn(
      `Google Analytics is not initialized. Event "${action}" was not sent.`,
      params,
    )
  }
}
