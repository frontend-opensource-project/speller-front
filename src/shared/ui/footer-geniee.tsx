...
const FooterGenieeSlot = ({ includeDevice }: { includeDevice: Breakpoint[] }) => {
  const {
  ...
  const handleAdUnfilled = useCallback(() => {
    if (retryCount < MAX_RETRIES) {
      console.warn(
        `🔁 Geniee ad unfilled. Retry attempt: ${retryCount + 1}/${MAX_RETRIES} for key ${adKey}`,
      )
      attemptRetry()
    } else {
      console.warn(
        `🛑 Max retries reached for Geniee ad key ${adKey}. Ad will be hidden.`,
      )
